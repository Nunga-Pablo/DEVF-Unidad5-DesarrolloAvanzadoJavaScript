        ...
        setTimeout(() => 
            {
                ...
                {
                    resolve("Cantidad de mesas disponibles");
                } 
                else 
                {
                    reject("No se cuenta con la cantidad de mesas disponibles para su reservación");
                }
...
function enviarConfirmacionReserva(nombreCliente) 
{
    return new Promise((resolve, reject) => 
    {
        setTimeout(() => 
        {
            ...
            if (exito) 
            {
                resolve(`Estimado ${nombreCliente}: Su reservación ha sido completada correctamente, saludos.`);
            } 
            else 
            {
                reject("Fallo en el servidor de correo.");
            }
...
hacerReserva("Juan Pérez", 3);  // Intenta hacer una reserva de 3 mesas

setTimeout(() => 
{
    hacerReserva("Pedro López", 6);  // Intenta hacer una reserva de 6 mesas
}, 5000);  // Simula una nueva reservacion despues de 5 segundos